...
async function enhanceImageWink(imagePath) {
    ...
    async function queryBatch(msgId, referer = `${BASE_URL}/image-enhancer/upload`) {
        const params = baseParams({
            msg_ids: msgId
        });
    ...
    const uploaded = await uploadToQiniu(policy, imagePath);
...
module.exports = {
    name: 'wink',
    command: ['wink', 'hd', 'enhance'],
    category: 'image',
    description: 'Meningkatkan resolusi/kualitas gambar menjadi Ultra HD',
    isGroup: false,
    isAdmin: false,
    isBotAdmin: false,
    execute: async (sock, m, { quotedMsg }) => {
        ...
        if (!imageMessage) {
            return await sock.sendMessage(jid, { text: '❌ Kirim gambar dengan caption *.wink* / *.hd* atau reply gambar dengan *.wink* / *.hd*!' }, { quoted: m });
        }
        ...
        const inputPath = path.join(tempDir, `wink_in_${Date.now()}.jpg`);

        try {
            await sock.sendMessage(jid, { text: '⏳ Sedang mengunduh gambar dan memproses peningkatan ke Ultra HD (Wink AI), mohon tunggu... (Proses ini memakan waktu sekitar 15-30 detik)' }, { quoted: m });
            
            const buffer = await downloadMedia(imageMessage, 'image');
            fs.writeFileSync(inputPath, buffer);

            const resultUrl = await enhanceImageWink(inputPath);

            await sock.sendMessage(jid, {
                image: { url: resultUrl },
                caption: '✨ *Wink Ultra HD Enhancer*\n\nGambar Anda berhasil ditingkatkan menjadi kualitas Ultra HD! 🚀'
            }, { quoted: m });

        } catch (err) {
            console.error('Wink Enhancer Error:', err);
            await sock.sendMessage(jid, { text: `❌ Terjadi kesalahan saat meningkatkan gambar: ${err.message}` }, { quoted: m });
        } finally {
