...
module.exports = {
    ...
    command: ["resize"],
    category: 'image',
    description: 'Ubah ukuran dimensi gambar dengan cepat',
    isGroup: false,
    isAdmin: false,
    isBotAdmin: false,
    execute: async (sock, m, { text, args, isGroup, sender, groupMetadata, config, dbHelper, quotedMsg }) => {
        ...
        try {
            ...
            const imageUrl = `https://api.vreden.web.id/api/maker-resize?image=${encodeURIComponent(text)}`;
            await sock.sendMessage(from, { 
                image: { url: imageUrl },
                caption: `✅ *Efek Resize Diterapkan*\n\nSumber: ${text}\n${PROMO_TEXT}`
            }, { quoted: m });
        } catch (err) {
