...
module.exports = {
    ...
    execute: async (sock, m, { quotedMsg }) => {
        ...
        if (!imageMessage) {
            return await sock.sendMessage(jid, { text: '❌ Kirim gambar dengan caption *.hd* atau reply gambar dengan *.hd*!' }, { quoted: m });
        }
        ...
        const inputPath = path.join(tempDir, `hd_in_${Date.now()}.jpg`);

        try {
            await sock.sendMessage(jid, { text: '⏳ Sedang mengunduh gambar dan memproses peningkatan ke Ultra HD, mohon tunggu... (Proses ini memakan waktu sekitar 15-30 detik)' }, { quoted: m });
            ...
            await sock.sendMessage(jid, {
                image: { url: resultUrl },
                caption: '✨ *Peningkat Ultra HD*\n\nGambar Anda berhasil ditingkatkan menjadi kualitas Ultra HD! 🚀'
            }, { quoted: m });

        } catch (err) {
            console.error('HD Enhancer Error:', err);
