...
module.exports = {
    ...
    command: ["cak-lontong"],
    category: 'game',
    description: 'Bermain game tebak tebakan kocak ala Cak Lontong',
    isGroup: false,
    isAdmin: false,
    isBotAdmin: false,
    execute: async (sock, m, { text, args, isGroup, sender, groupMetadata, config, dbHelper, quotedMsg }) => {
        ...
        const question = 'Lari cepat jarak jauh disebut... (Jawaban: capek)';
        const answer = 'capek';
        ...
        setTimeout(() => {
            if (sock.gameSession[from] && sock.gameSession[from].type === 'caklontong') {
                sock.sendMessage(from, { text: `⏱️ Waktu habis! Jawabannya adalah: *capek*` });
                delete sock.gameSession[from];
            }
        }, 60000);

        await sock.sendMessage(from, { text: `🎮 *GAME CAKLONTONG*\n\n${question}\n\nJawab langsung dengan mengetik jawabannya (tanpa prefix).\nWaktu menjawab: 60 detik.` }, { quoted: m });
