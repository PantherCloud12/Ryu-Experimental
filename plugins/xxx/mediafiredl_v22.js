...
module.exports = {
    name: 'mediafiredl V22',
    command: ["mf22","mediafire22"],
    category: 'downloader-pro',
    description: 'Auto download file dari Mediafire (Versi 22)',
    execute: async (sock, m, { text, config, quotedMsg }) => {
        ...
        try {
            if (['hd-tools'].includes('downloader-pro')) {
                ...
                await sock.sendMessage(from, { text: '🔄 *Sedang diproses... (V22)*' }, { quoted: m });
                const buffer = await downloadMedia(media, 'image');
                await sock.sendMessage(from, { image: buffer, caption: '✅ Berhasil diproses dengan mediafiredl V22' }, { quoted: m });
            } else if (['downloader-pro'].includes('downloader-pro')) {
                if (!text) return sock.sendMessage(from, { text: '❌ Masukkan URL target!' }, { quoted: m });
                await sock.sendMessage(from, { text: '🔄 *Mencoba mendownload... (V22)*' }, { quoted: m });
                ...
                await sock.sendMessage(from, { text: '🤖 *AI sedang berpikir... (V22)*' }, { quoted: m });
                await sock.sendMessage(from, { text: '✅ Hasil: Ini adalah jawaban dari mediafiredl V22 menggunakan ' + baseUrl }, { quoted: m });
            }
        } catch (e) {
