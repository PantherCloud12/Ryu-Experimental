...
module.exports = {
    ...
    command: ["dnslookup"],
    category: 'tools',
    description: 'Melakukan DNS lookup IP record pada domain',
    isGroup: false,
    isAdmin: false,
    isBotAdmin: false,
    execute: async (sock, m, { text, args, isGroup, sender, groupMetadata, config, dbHelper, quotedMsg }) => {
        ...
        try {
            ...
            const res = await axios.get(`https://api.agatz.xyz/api/dns?domain=${encodeURIComponent(text)}`);
            ...
            await sock.sendMessage(from, { text: `🔍 *HASIL PENCARIAN DNS*\n\n${replyText}` }, { quoted: m });
        } catch (err) {
