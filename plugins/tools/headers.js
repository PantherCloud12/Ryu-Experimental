...
module.exports = {
    ...
    command: ["httpheaders"],
    category: 'tools',
    description: 'Mendapatkan HTTP response headers dari link web target',
    isGroup: false,
    isAdmin: false,
    isBotAdmin: false,
    execute: async (sock, m, { text, args, isGroup, sender, groupMetadata, config, dbHelper, quotedMsg }) => {
        ...
        try {
            ...
            const res = await axios.get(`https://api.agatz.xyz/api/headers?url=${encodeURIComponent(text)}`);
            ...
            await sock.sendMessage(from, { text: `🔍 *HASIL PENCARIAN HEADERS*\n\n${replyText}` }, { quoted: m });
        } catch (err) {
